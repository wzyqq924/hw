...
choices = ['華碩: 2023.4.17 至 2025.4.16', '元大台灣50正2: 2023.4.17 至 2025.4.17', '聯電期貨: 2023.4.17 至 2024.4.16']
choice = st.selectbox('選擇金融商品', choices, index=0)
##### 读取Pickle文件
if choice == choices[0] :         ##'台積電: 2022.1.1 至 2024.4.9':
    df_original = load_data('kbars_1d_2357_2023-04-17_To_2025-04-16.pkl')
    product_name = '華碩'
...
if choice == choices[2] :                              ##'小台指期貨2024.12到期: 2023.12 至 2024.4.11':
    df_original = load_data('kbars_1d_CCF_2023-04-17_To_2025-04-16.pkl')
    product_name = '聯電期貨'
...
choices_strategies = [
    '<進場>: MA黃金交叉多單, 死亡交叉空單.',
    '<進場>: RSI低於30反轉作多, RSI高於70反轉作空.',
    '<進場>: 布林下緣反彈作多, 上緣反轉作空.',
    '<進場>: MACD黃金交叉作多, 死亡交叉作空.'
]
choice_strategy = st.selectbox('選擇交易策略', choices_strategies, index=0)
...
if choice_strategy == choices_strategies[1]:
    with st.expander('<策略參數設定>: RSI閾值 & 移動停損'):
        RSI_floor = st.slider("RSI地板(作多)", 0, 100, 30)
        RSI_ceil = st.slider("RSI天花板(作空)", 0, 100, 70)
        MoveStopLoss = st.slider("移動停損", 0, 100, 30)
        Order_Quantity = st.slider("交易數量", 1, 10, 1)

    for n in range(1, len(KBar_df)-1):
        if not np.isnan(KBar_df['RSI_short'][n-1]):
            if OrderRecord.GetOpenInterest() == 0:
                if KBar_df['RSI_short'][n] < RSI_floor:
                    OrderRecord.Order('Buy', KBar_df['product'][n+1], KBar_df['time'][n+1], KBar_df['open'][n+1], Order_Quantity)
                    StopLossPoint = KBar_df['open'][n+1] - MoveStopLoss
                elif KBar_df['RSI_short'][n] > RSI_ceil:
                    OrderRecord.Order('Sell', KBar_df['product'][n+1], KBar_df['time'][n+1], KBar_df['open'][n+1], Order_Quantity)
                    StopLossPoint = KBar_df['open'][n+1] + MoveStopLoss
            elif OrderRecord.GetOpenInterest() > 0:
                if KBar_df['close'][n] - MoveStopLoss > StopLossPoint:
...
# 布林通道策略
    ...
    with st.expander('<策略參數設定>: 布林觸發反轉 & 移動停損'):
        MoveStopLoss = st.slider("移動停損", 0, 100, 30)
        Order_Quantity = st.slider("交易數量", 1, 10, 1)

    for n in range(1, len(KBar_df)-1):
        if not np.isnan(KBar_df['Lower_Band'][n-1]):
            if OrderRecord.GetOpenInterest() == 0:
...
# MACD策略
    ...
    with st.expander('<策略參數設定>: MACD金叉死叉 & 移動停損'):
        MoveStopLoss = st.slider("移動停損", 0, 100, 30)
        Order_Quantity = st.slider("交易數量", 1, 10, 1)

    for n in range(1, len(KBar_df)-1):
        if not np.isnan(KBar_df['MACD'][n-1]):
            if OrderRecord.GetOpenInterest() == 0:
...
def 計算績效_期貨():
    交易總盈虧 = OrderRecord.GetTotalProfit()*2000          ## 取得交易總盈虧
    平均每次盈虧 = OrderRecord.GetAverageProfit()*2000         ## 取得交易 "平均" 盈虧(每次)
    平均投資報酬率 = OrderRecord.GetAverageProfitRate()    ## 取得交易 "平均" 投資報酬率(每次)  
    平均獲利_只看獲利的 = OrderRecord.GetAverEarn()*2000              ## 平均獲利(只看獲利的) 
    平均虧損_只看虧損的 = OrderRecord.GetAverLoss()*2000             ## 平均虧損(只看虧損的)
    勝率 = OrderRecord.GetWinRate()              ## 勝率
    最大連續虧損 = OrderRecord.GetAccLoss()*2000               ## 最大連續虧損
    最大盈虧回落_MDD = OrderRecord.GetMDD()*2000                   ## 最大利潤(盈虧)回落(MDD). 這個不是一般的 "資金" 或 "投資報酬率" 的回落
    if 最大盈虧回落_MDD>0:
        報酬風險比 = 交易總盈虧/最大盈虧回落_MDD
    else:
        報酬風險比='資料不足無法計算'
